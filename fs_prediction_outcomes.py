...
status_map = df_DC.set_index('ID')['Status']
df_FC['Status'] = df_FC['ID'].map(status_map)
df_SC['Status'] = df_SC['ID'].map(status_map)
df_DC = df_DC[df_DC['Status'].notna()].reset_index(drop = True)
df_FC = df_FC[df_FC['Status'].notna()].reset_index(drop = True)
df_SC = df_SC[df_SC['Status'].notna()].reset_index(drop = True)
...
for i, metric in enumerate(metrics):
    for modality in ['FC-SC','DC']:
        if modality == 'DC' and metric == 'PC':
            df_filtered = df_DC
            ...
            df_filtered = pd.concat([df_filtered.iloc[:,:8].reset_index(drop = True), 
                                     df_filtered_net_FC.reset_index(drop = True), 
                                     df_filtered_net_SC.reset_index(drop = True)], axis = 1)
                                         
        test_results, shap_values_sets, odds_ratio_sets, features = fs_classifier_cv_parallel(df_filtered, metric, modality)
        ...
        test_results.to_csv(join(maindir,'Results','fs_prediction','tables',f'{metric}_{modality}_st_outcomes_ncv_HCP_MMP1_DK_all.csv'))
        feature_importance_df.to_csv(join(maindir,'Results','fs_prediction','tables',f'FI_{metric}_{modality}_st_outcomes_ncv_HCP_MMP1_DK_all.csv'))
