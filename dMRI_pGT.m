...
% obtaining a set of regions used for SC
...
% missing nodes
fid = fopen('dMN_prediction.txt');
data = textscan(fid, '%s %s','Delimiter','\t','HeaderLines',1);
fclose(fid);
MN = cell(length(data{2}), 1);
for i = 1:length(data{2})
    if isempty(data{2}{i})
        MN{i} = [];
    else
        nums = str2double(strtrim(strsplit(data{2}{i}, ',')));
        MN{i} = nums;
    end
end
MN = [data{1}, MN];
...
%% ========================================================================
% load a SC structure for all participants
load([datadir,'/', atlas '_SC_matrix_prediction.mat']);
for thr = 5:5:25
...
writetable(GT_all_table,[datadir,'/','GT_all_',atlas,'_prediction_mean.csv']);
...
for GT_metric = GT_metrics
    ...
    covars_list = py.dict(pyargs('Site',covars(1,:)));
...
GT_all_table_MI_H = cell2table(data_MI_H,'VariableNames',['ID','Status','Site','Epilepsy Side','Type of surgery','Surgery Side','Gender','Age','AgeOnset',...
                                                          ...
                                                          'iEEG hemisphere','iEEG electrode type','iEEG electrode number','iEEG days of monitoring','iEEG percentage of spikes',...
                                                           'GM volume','WM volume','GT metrics',masks']);
writetable(GT_all_table_MI_H,[datadir,'/','GT_all_',atlas,'_prediction_mean_MI_H.csv']);

%% ========================================================================
% atypical hubness
data_MI_H_normative = table2cell(readtable([maindir(1:id(end) - 15),'/cMRI_BIDS','/Data','/ImageDatabase_BIDS','/Controls',...
                                                                    '/derivatives','/MRtrix','/GT_all_HCP_MMP1_DK_ahubness_mean_MI_H.csv']));
...
GT_all_table_MI_H_atypical = cell2table(data_MI_H_atypical,'VariableNames',['ID','Status','Site','Epilepsy Side','Type of surgery','Surgery Side','Gender','Age','AgeOnset',...
                                                                            ...
                                                                            'iEEG hemisphere','iEEG electrode type','iEEG electrode number','iEEG days of monitoring','iEEG percentage of spikes',...
                                                                            'GM volume','WM volume','GT metrics',masks']);
writetable(GT_all_table_MI_H_atypical,[datadir,'/','GT_all_',atlas,'_prediction_mean_MI_H_atypical.csv']);
...
writetable(data_arranged,[datadir,'/GT_all_',atlas,'_prediction_mean_MI_H_atypical_ipsicontra.csv']);