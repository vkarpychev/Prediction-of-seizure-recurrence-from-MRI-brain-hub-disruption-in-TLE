...
for i = 1:length(data{2})
    if isempty(data{2}{i})
        ...
        nums = str2double(regexp(data{2}{i},'[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?', 'match'));
        MN{i} = nums;
    end
end
MN = [data{1}, MN];
...
load([datadir,'/', atlas '_SC_matrix_ahubness.mat']);
for thr = 5:5:25
...
writetable(GT_all_table,[datadir,'/','GT_all_',atlas,'_ahubness_mean.csv']);
...
writetable(GT_all_table_MI_H,[datadir,'/','GT_all_',atlas,'_ahubness_mean_MI_H.csv']);
