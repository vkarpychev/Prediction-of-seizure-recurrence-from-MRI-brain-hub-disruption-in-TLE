...
for i, metric in enumerate(metrics):
    for modality in ['FC-SC','FC','SC','DC']:
        if modality == 'DC' and metric == 'BC':
          ...
          #  CH_FC_group_hubs = symmetrical_brain_hubs_FC.copy()
          #  CH_FC_group = sio.loadmat(join('/media/rgong/cMRI_BIDS/code/fs_ahubness', f"FC_{metric}_CH_group.mat"))['CH_group'].flatten()   
          #  CH_FC_group_hubs[CH_FC_group_hubs == 1] = CH_FC_group
          ...
          #  CH_FC_group_hubs = np.concatenate([CH_FC_group_hubs[0:180], CH_FC_group_hubs[360:367], 
          #                                     CH_FC_group_hubs[180:360], CH_FC_group_hubs[367:]])
          #  CH_FC_group_hubs = CH_FC_group_hubs[CH_FC_group_hubs != 0]
           ...
           # for n in np.unique(CH_FC_group_hubs):
           #      df_filtered_net_FC[f'CH_FC_group_{n}'] = df_filtered_FC[np.array(df_filtered_FC.columns)[CH_FC_group_hubs == n]].mean(axis = 1)
           ...
           # CH_SC_group_hubs = symmetrical_brain_hubs_SC.copy()
           # CH_SC_group = sio.loadmat(join('/media/rgong/cMRI_BIDS/code/fs_ahubness', f"SC_{metric}_CH_group.mat"))['CH_group'].flatten()   
           # CH_SC_group_hubs[CH_SC_group_hubs == 1] = CH_SC_group
           ...
           # CH_SC_group_hubs = np.concatenate([CH_SC_group_hubs[0:180], CH_SC_group_hubs[360:367], 
           #                                    CH_SC_group_hubs[180:360], CH_SC_group_hubs[367:]])
           # CH_SC_group_hubs = CH_SC_group_hubs[CH_SC_group_hubs != 0]
            ROI_cols = df_filtered.columns[9:]
           ...
           # for n in np.unique(CH_SC_group_hubs):
           #      df_filtered_net_SC[f'CH_SC_group_{n}'] = df_filtered_SC[np.array(df_filtered_SC.columns)[CH_SC_group_hubs == n]].mean(axis = 1)
            ...
            # hub_cols = np.array(ROI_FC_cols)[np.array(symmetrical_brain_hubs_FC == 1).flatten()]
            # hub_subcortical = np.array([any(k in col for k in subcortical_areas) for col in hub_cols])
            # cortical_hub_cols = hub_cols[~ hub_subcortical]
            # subcortical_hub_cols = hub_cols[hub_subcortical]
            # cortical_hub_groups = CH_FC_group_hubs[~ hub_subcortical]
            # cortical_hub_groups = Ci_FC[~ hub_subcortical]
            # df_filtered_FC_cortical = df_filtered[cortical_hub_cols].reset_index(drop = True)
            # df_filtered_FC_subcortical = df_filtered[subcortical_hub_cols].reset_index(drop = True)
            ...
            for n in np.unique(Ci_FC):
                 df_filtered_net_FC[f'FC_group_{n}'] = df_filtered_FC[np.array(df_filtered_FC.columns)[Ci_FC == n]].mean(axis = 1)
            # for n in np.unique(cortical_hub_groups):
            #         df_filtered_net_FC[f'CH_FC_group_{n}'] = df_filtered_FC_cortical.iloc[:, cortical_hub_groups == n].mean(axis = 1)
            df_filtered_net_FC = pd.DataFrame(df_filtered_net_FC)
            # df_filtered_net_FC = pd.concat([df_filtered_net_FC, df_filtered_FC_subcortical], axis = 1)
            ...
            # hub_cols = np.array(ROI_SC_cols)[np.array(symmetrical_brain_hubs_SC == 1).flatten()]
            # hub_subcortical = np.array([any(k in col for k in subcortical_areas) for col in hub_cols])
            # cortical_hub_cols = hub_cols[~ hub_subcortical]
            # subcortical_hub_cols = hub_cols[hub_subcortical]
            # cortical_hub_groups = CH_SC_group_hubs[~ hub_subcortical]
            # cortical_hub_groups = Ci_SC[~ hub_subcortical]
            # df_filtered_SC_cortical = df_filtered[cortical_hub_cols].reset_index(drop = True)
            # df_filtered_SC_subcortical = df_filtered[subcortical_hub_cols].reset_index(drop = True)
            ...
            for n in np.unique(Ci_SC):
                 df_filtered_net_SC[f'SC_group_{n}'] = df_filtered_SC[np.array(df_filtered_SC.columns)[Ci_SC == n]].mean(axis = 1)
            # for n in np.unique(cortical_hub_groups):
            #          df_filtered_net_SC[f'CH_SC_group_{n}'] = df_filtered_SC_cortical.iloc[:, cortical_hub_groups == n].mean(axis = 1)
            df_filtered_net_SC = pd.DataFrame(df_filtered_net_SC)
            # df_filtered_net_SC = pd.concat([df_filtered_net_SC, df_filtered_SC_subcortical], axis = 1)
